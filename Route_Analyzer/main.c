...
static void build_graph_from_user(Graph *g) {
  ...
  printf(COLOR_CYAN "\nEnter number of nodes (max %d): " COLOR_RESET,
         MAX_NODES);
  ...
  printf(COLOR_BOLD "from to distance traffic_factor weather_factor "
  ...
  for (i = 0; i < num_edges; i++) {
    int from, to;
    double distance;
    double traffic_factor, weather_factor, road_condition_factor,
        temperature_factor;

    printf("Edge %d: ", i + 1);
    if (scanf("%d %d %lf %lf %lf %lf %lf", &from, &to, &distance,
              &traffic_factor, &weather_factor, &road_condition_factor,
              &temperature_factor) != 7) {
      printf(COLOR_RED "[!] Invalid input format for edge. Stopping edge "
                       "input.\n" COLOR_RESET);
      clear_input_buffer();
      break;
    }

    add_edge(g, from, to, distance, traffic_factor, weather_factor,
             road_condition_factor, temperature_factor);
  }
...
static void load_graph_from_file(Graph *g) {
  ...
  if (fscanf(f, "%d %d", &num_nodes, &num_edges) != 2) {
    printf(COLOR_RED
           "[!] Invalid file format. Expected: <nodes> <edges>\n" COLOR_RESET);
    fclose(f);
    return;
  }
  ...
  for (int i = 0; i < num_edges; i++) {
    ...
    if (fscanf(f, "%d %d %lf %lf %lf %lf %lf", &from, &to, &dist, &trafic,
               &weath, &road, &temp) == 7) {
  ...
  fclose(f);
  printf(COLOR_GREEN "\n[+] Successfully loaded graph with %d nodes and %d "
                     "edges from '%s'.\n" COLOR_RESET,
         num_nodes, loaded_edges, filename);
}
...
static void display_detailed_path(const Graph *g, int parent[], int dest,int source) {
  ...
  for (int i = count - 1; i > 0; i--) {
    ...
    if (edge) {
      ...
      printf("  " COLOR_YELLOW "↓" COLOR_RESET " Take road to " COLOR_BOLD
             "%s" COLOR_RESET " (Dist: " COLOR_MAGENTA "%.1fkm" COLOR_RESET
             ", Traffic: %.1fx, Weather: %.1fx)\n",
             g->node_names[v], edge->distance, edge->traffic_factor,
             edge->weather_factor);
    } else {
...
static void run_shortest_path(const Graph *g) {
  if (g->num_nodes <= 0) {
    printf(COLOR_RED "\n[!] Graph is empty. Please load or create a road "
                     "network first.\n" COLOR_RESET);
    return;
  }
  ...
  int source, dest;
  printf(COLOR_CYAN "\nEnter source location ID (0 to %d): " COLOR_RESET,
         g->num_nodes - 1);
  ...
  printf(COLOR_CYAN "Enter destination location ID (0 to %d): " COLOR_RESET,
         g->num_nodes - 1);
  ...
  if (!bellman_ford(g, source, dist, parent)) {
    printf(COLOR_RED "\n[!] CRITICAL ERROR: Negative weight cycle detected in "
                     "road network!\n" COLOR_RESET);
    printf(COLOR_YELLOW
           "This means some road factors are causing infinite travel cost "
           "loops. Please check your data.\n" COLOR_RESET);
    return;
  }

  if (dist[dest] >= INF / 2) {
    printf(COLOR_RED "\n[!] No path exists from %d to %d (Roads might be "
                     "disconnected or one-way).\n" COLOR_RESET,
           source, dest);
    return;
  }
...
static void show_menu(void) {
  ...
  printf("  " COLOR_YELLOW "1." COLOR_RESET
         " Create/Update Road Network Manually\n");
  printf("  " COLOR_YELLOW "2." COLOR_RESET
         " Load Road Network from File (e.g. map.txt)\n");
  printf("  " COLOR_YELLOW "3." COLOR_RESET
         " Display Current Road Network Layout\n");
  printf("  " COLOR_YELLOW "4." COLOR_RESET
         " Find Shortest Path (Turn-by-Turn GPS)\n");
  printf("  " COLOR_YELLOW "5." COLOR_RESET
         " Update Specific Road Conditions Dynamically\n");
  printf("  " COLOR_YELLOW "6." COLOR_RESET
         " Show Rating Parameters/Factors Help\n");
